<?php
    ...
    if(!empty($_POST)) {
        ...
        $myHost->setCode(HostValidator::slugifyData($_POST['code']));
        ...
        if ($myArray['isSuccess'] == 0){ //si l'attribut isSuccess de myArray est 0 alors validatesCustomer n'a trouvé aucune erreur
            HostRepository::insertHost($myHost);
...
<html>
    ...
    <?php include '../Layout/Menu.php'; ?>
        ...
        <div class="container admin">
            <div class="row">
                <h1><strong>Ajouter un Hébergeur</strong></h1>
                <br>
                <form class="form" role="form" method="post">
                    <br>
                    <div>
                        <label class="form-label" for="code">Code :</label>
                        <input type="text" class="form-control" id="code" name="code" placeholder="Insérer un code pour l'hébergeur" value="<?php echo $myHost->getCode();?>">
                        <span class="help-inline"><?php echo $myArray['codeError'];?></span>
                    </div>
                    <br>
                    <div>
                        <label class="form-label" for="name">Nom :</label>
                        <input type="text " class="form-control" id="name" name="name" placeholder="Insérer un nom pour l'hébergeur" value="<?php echo $myHost->getName();?>">
